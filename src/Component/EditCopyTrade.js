...
const EditAutoCopyTrade = ({ children, id, callback }) => {
  ...
  const getsingleTrade = (_id) => {
    setLoading(true);
    axios
      .get(`https://prolivemarkets-ykwsl.ondigitalocean.app/api/autocopytrade/single/${_id}`)
      .then(
        (response) => {
          setLoading(false);
  ...
  const EditTrade = (_id) => {
    ...
    axios
      .put(`https://prolivemarkets-ykwsl.ondigitalocean.app/api/autocopytrade/updatetrade/${_id}`, data)
      .then(
        (response) => {
          handleCancel();
